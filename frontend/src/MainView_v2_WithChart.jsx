...
const MainView_v2_WithChart = () => {
  ...
  const [httpErrorPlotChart, setHttpErrorPlotChart] = useState(null);
  const [error, setError] = useState(null);
  const [cleanliness, setCleanliness] = useState("");
  const [uniqueness, setUniqueness] = useState("");
  const [deletionID, setDeletionID] = useState(null);
  const [deleteDocsError, setDeleteDocsError] = useState(null);
  const [nonUnderscoredIdDeletionResult, setNonUnderscoredIdDeletionResult] =
    useState("");
  ...
  ] = useState("<Document deletion status will appear here>");
  const [notifyDeletingRedundantDocs, setNotifyDeletingRedundantDocs] =
    useState("<Document deletion status will appear here>");
  const [
    docsDeletionDetailsSavedToDBStatus,
    setDocsDeletionDetailsSavedToDBStatus,
  ] = useState(null);
  ...
  useEffect(() => {
    const fetchCount = async () => {
      try {
        ...
        const response1 = await fetch(
          "http://localhost:6060/api/countDocsWithoutUnderscoreID"
        );
        ...
        const response2 = await fetch(
          "http://localhost:6060/api/countRedundantDocs"
        );
        const result2 = await response2.json();
        setRedundantDocsCount(result2.count);
      } catch (error) {
        console.error(
          "Error fetching non-underscored ID or redundant count:",
          error
        );
      }
    };
  ...
  useEffect(() => {
    const sendIntervalToBackend = async () => {
      try {
        ...
        console.log(
          "send delete docs interval to backend status: " + result.message
        );
      } catch (error) {
    ...
    const deleteNonUnderscoredDocuments = async () => {
      ...
      setNotifyDeletingNonUnderscoredIdDocs(
        "Deleting non-underscored ID documents..."
      );
      await delay(2000);
      try {
        ...
        if (response.status === 200) {
          ...
          setNotifyDeletingNonUnderscoredIdDocs(
            "Deleting non-underscored ID documents completed"
          );
          await delay(2000);
          setNotifyDeletingNonUnderscoredIdDocs(
            "<Document deletion status will appear here>"
          );
        }
      } catch (error) {
    ...
    if (nonUnderscoredIdDocsCount > 0) deleteNonUnderscoredDocuments();
    ...
    const deleteRedundantDocuments = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          setNotifyDeletingRedundantDocs(
            "Deleting redundant documents completed"
          );
          await delay(2000);
          setNotifyDeletingRedundantDocs(
            "<Document deletion status will appear here>"
    ...
    if (redundantDocsCount > 0) deleteRedundantDocuments();
  ...
  useEffect(() => {
    const recordRecentDocsDeletionDetails = async () => {
      try {
        const response = await fetch(
          "http://localhost:7070/api/recordRecentDocsDeletionDetails",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              _id: deletionID,
              deletion_type:
                deletionID === "01" ? "non-underscore ID" : "redundant",
              last_deletion_date: format(
        ...
        const result = await response.json();
        setDocsDeletionDetailsSavedToDBStatus(
          `Successful recorded docs deletion details: ${result.message}`
        );
      } catch (error) {
  ...
  useEffect(() => {
    nonUnderscoredIdDocsCount === 0
      ? setCleanliness("Clean")
      : setCleanliness("Unclean");
    redundantDocsCount === 0
      ? setUniqueness("Unique")
      : setUniqueness("Non-unique");
  }, [nonUnderscoredIdDocsCount, redundantDocsCount]);
  ...
  /*  Extracting data from DB for plotting line chart  */
  useEffect(() => {
    const fetchDataForPlottingChart = async () => {
      try {
        const response = await fetch("http://localhost:6060/api/get2025Data");
        if (!response.ok) {
          setHttpErrorPlotChart(`HTTP error! status: ${response.status}`);
        } else {
          const data = await response.json();
          setHttpErrorPlotChart(null);
          setData(data);
          setLoading(false);
        }
      } catch (error) {
        setHttpErrorPlotChart(error.message + ". Please check your database connection");
        setLoading(false);
      }
    };

    fetchDataForPlottingChart();
  }, []);

  const processDataForChart = (data) => {
    const hourlyData = {};
    data.forEach(item => {
      const hour = item.hourlyTF;
      const coins = item.coins;
      for (const coin in coins) {
        if (!hourlyData[coin]) {
          hourlyData[coin] = [];
        }
        hourlyData[coin].push({ hour, priceChangePercent: coins[coin] });
      }
    });

    const chartsData = Object.keys(hourlyData).map(coin => {
      const sortedData = hourlyData[coin].sort((a, b) => a.hour.localeCompare(b.hour));
      return {
        labels: sortedData.map(item => item.hour),
        datasets: [
          {
            label: `${coin} Price Change Percent`,
            data: sortedData.map(item => item.priceChangePercent),
            fill: false,
  ...
  const chartsData = processDataForChart(data);
        ...
        <div className="Statistics">
          {tab === 1 ? (
            <DetailedStats
              ...
              data={data}
              dataSavedToDBStatus={
                dataSavedToDBStatus ? dataSavedToDBStatus : error
              }
            ...
            <Chart
              ...
              options={options}
              httpErrorPlotChart={httpErrorPlotChart}
            />
          )}
        </div>
      </>
...
export default MainView_v2_WithChart;